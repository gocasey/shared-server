...
function ApplicationUserTokenSchemaValidator(logger) {
  let _logger = logger;

  this.validateRequest = function(req, res, next) {
    _logger.debug('Request received: %j', req.body);
    let validator = new SchemaValidator();
    validator.validateJson(req.body, 'application_user_token_request.json', function(err) {
      if (err) {
...
module.exports = ApplicationUserTokenSchemaValidator;
